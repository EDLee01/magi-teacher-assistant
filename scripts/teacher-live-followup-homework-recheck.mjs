...
process.env.MAGI_TEACHER_HOMEWORK_ASSERTIONS_ONLY = "1";
const { evaluateHomeworkResult } = await import("./teacher-live-followup-homework-eval.mjs");
...
const recheckReportPath = path.join(
  repositoryRoot,
  ".magi-reports",
  "teacher-live-followup-homework-recheck.json"
);
let runtime;
...
try {
  ...
  const candidates = buildPhysicsQuestionCandidatePack({
    ...
    query: "滑动摩擦力 浮力 原题 真题 作业 练习 答案 解析 复测 8题分层作业",
    limit: 36
  });
  ...
  process.stderr.write(
    `[追问作业离线复核] 失败：${error instanceof Error ? error.stack || error.message : String(error)}\n`
  );
  process.exitCode = 1;
} finally {
  runtime?.close();
