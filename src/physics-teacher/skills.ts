...
1. 只使用项目中已有且获准使用的学生数据，优先使用匿名编号；先说明证据覆盖到班级、小组还是个人，不在成果中扩散姓名、联系方式或与教学无关的个人信息。
2. 每个学生或小组先列“数据事实—教学解释—证据不足/仍需核实”，再按“当前会什么—卡在哪里—下一小步—怎样证明学会”描述学习需要。一次作答、缺交或缺测不能升级为稳定能力结论，不贴“学困生、差生、优秀生、后进生”等固定标签。
3. 每名学生或每组最多安排一到两个近期目标。目标必须由其自己的证据支持；已掌握的基础内容不机械重做，缺少关键数据时先补测或收集证据，再决定补弱方向。
4. 学习路径要落实到可执行日程。逐日写清活动、材料/原题、预计用时、学生产出、教师观察信号和完成标准；每天总量要适合正常教学节奏，不用题量制造“个性化”。
5. 需要练习时与物理命题 Skill 配合：优先分配题干完整、答案可核对且图片可交付的题库原题，保留来源文件、原题号和资料 ID。原题可以因目标匹配而跨学生复用，但不能给每个人机械发同一套题。
6. 路径结束必须有短复测和调整规则。写明达到、部分达到、未达到分别进入什么下一步；新证据与原判断冲突时，以新证据调整目标，不维护旧标签。
7. 单独列出教师需要确认的分组、目标、题目和时间安排。涉及长期学情结论时，只在成果中生成“待教师确认的记忆草稿”；未经教师确认不得写入长期项目记忆，也不得把一次临时表现写成长期事实。

## 输出

- 数据范围、隐私边界与证据表
- 每个匿名对象的当前状态与近期目标
- 7 天学习路径表（活动、材料、用时、产出、观察信号、完成标准）
- 原题任务与来源表
- 短复测、调整规则和教师跟踪记录
- 待教师确认的问题
- 待教师确认的记忆草稿（不自动写入）
...
function isManagedPhysicsTeacherSkill(content: string, name: string): boolean {
  ...
  if (name === "physics-personalized-learning") {
    return (
      content.includes("PERSONALIZED_LEARNING_BUSINESS_MARKER") ||
      (content.includes("# 物理个性化学习") &&
        content.includes("按“当前会什么—卡在哪里—下一小步—怎样证明学会”"))
    );
  }
  return false;
}
